...
int main(int argc, char **argv)
{
    ...
    float A_ped, B_ped, A_people, B_people;
    p->getValue("A_ped", A_ped);
    p->getValue("B_ped", B_ped);
    p->getValue("A_people", A_people);
    p->getValue("B_people", B_people);
    ...
    VideoWriter video("../Demo_ReId.avi", cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), 10, Size(frame_width, frame_height), true);
    ...
    while ((cap >> frame).grab())
    {
        ...
        case DETECT:
            ...
            {
                pedestrian_cascade.detectMultiScale(frame, ROIs, A_ped, B_ped, 0 | CASCADE_DO_CANNY_PRUNING, Size(50, 50));
            ...
            {
                fullbody_cascade.detectMultiScale(frame, ROIs, A_people, B_people, 0 | CASCADE_DO_CANNY_PRUNING, Size(50, 50));
                if (ROIs.empty() & classifier_counter == max_frame_try)
                {
        ...
        case TRACK:
            ...
            if (it_story >= 3)
            {
                ...
                if (delta < 10)
                {
                    ...
                    fullbody_cascade.detectMultiScale(frame, TMP_ROIs, 1.05, 6, 0 | CASCADE_DO_CANNY_PRUNING, Size(50, 50));
        ...
        case LOST_TRACK:
            ...
            if (classifierType == classifierTypes[0])
            {
                pedestrian_cascade.detectMultiScale(frame, ROIs, 1.05, 28, 0 | CASCADE_DO_CANNY_PRUNING, Size(50, 50));
